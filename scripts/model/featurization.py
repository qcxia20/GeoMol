...
class geom_confs(Dataset):
    ...
    def featurize_mol(self, mol_dic):
        ...
        for conf in confs:
            ...
            if conf_canonical_smi != canonical_smi: # Smiles from rdkit mol (conf) in
                continue
...
def featurize_mol_from_smiles(smiles, dataset='qm9'):
    # The information of SMILES is processed and extracted through RDKit
    if dataset == 'qm9':
        types = qm9_types
    elif dataset == 'drugs':
        types = drugs_types
    ...
    # filter mols rdkit can't intrinsically handle
    mol = Chem.MolFromSmiles(smiles) # 2D mol
    if mol:
        mol = Chem.AddHs(mol) # Before transmitting into neural network, hydrogens will be added by rdkit first to give corresponding information
    else:
        return None
    N = mol.GetNumAtoms()

    # filter out mols model can't make predictions for, which have no rotatable bonds
    if not mol.HasSubstructMatch(dihedral_pattern): # Need to have dihedral angle
        return None
    if N < 4:
        return None
